...
def sheet_cal(width, thickness, l_speed, s_density):
    ...
    if not isinstance(width, int):
        if not isinstance(width, float):
            raise TypeError("'width' should be either integer or float")
    ...
    cross_area = width * thickness / 1000000
    req_throughput = l_speed * cross_area * 60 * s_density
    
    return round(req_throughput, 3)
  

def sheet_table(
    width, thickness, s_density, density_ratio=0.85, min_l_speed=1, 
