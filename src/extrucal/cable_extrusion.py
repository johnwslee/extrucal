...
def req_throughput_cal(outer_d, thickness, l_speed, s_density):
    """
    Calculates the required throughput given the outer diameter, 
    inner diameter, line speed, and solid polymer density
    ...
    Examples
    --------
    >>> req_throughput_cal(outer_d=10, thickness=2, l_speed = 10, density=1000)
    """
    # Test input type
...
def req_rpm_table(
    ...
    Examples
    --------
    >>> req_rpm_table(outer_d=10, thickness=2, density=1000)
    """
    # Test input type
    if not isinstance(outer_d, int):
        if not isinstance(outer_d, float):
    ...
    for l in l_speed:
        for s in size:
            rpm_list.append(req_throughput_cal(outer_d, thickness, l, s_density)/throughput_cal(s, s*depth_percent, s_density*density_ratio))
...
def req_rpm_plot(
    ...
    Examples
    --------
    >>> req_rpm_plot(outer_d=10, thickness=2, density=1000)
    """
    # Test input type
    if not isinstance(outer_d, int):
        if not isinstance(outer_d, float):
